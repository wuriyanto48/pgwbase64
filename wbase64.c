...
Datum pgw_b64_encode(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(pgw_b64_decode);
Datum pgw_b64_decode(PG_FUNCTION_ARGS)
{
    text* input_text = PG_GETARG_TEXT_PP(0);
    int32  text_size = VARSIZE_ANY_EXHDR(input_text);
    if (text_size <= 0) 
    ...
    unsigned char* dst = NULL;
    size_t dst_size = 0;
    int encode_ok = decode_b64(input_text_cleaned, 0, &dst, &dst_size);
    if (encode_ok != 0)
    {
        ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
